...
已知 先序 中序求后序 		PS：（紫书P157 有讲解 中序后序求先序的例子）
可以通过先序的第一个字符，从中序中确定左节点和右节点的部分
然后把中序中的确定部分的数量确定先序中左树右树的部分

例 DBACEGF ABCDEFG 
因为先序最先输出的是中间节点，而中序是左节点，中序要先把左节点所有全部输出完才输出右边部分
因此先序的第一个字符D 可以把中序分成左右两部分ABC EFG
可以把BAC和ABC EGF和EFG这左树的先序中序，右树的先序中序继续递归的遍地下去
